...
import { getNotasEvaluacionAction, getEstudiantesCursoAction } from "@/actions/evaluations"
import prisma from "@/lib/prisma"
import { NotasForm } from "@/components/evaluaciones/notas-form"
...
export default async function NotasPage({ params }: NotasPageProps) {
  ...
  const notasMap = notasExistentes.reduce((acc: Record<string, { valor: number; valorLiteral?: string }>, nota: any) => {
    ...
    <div className="container mx-auto py-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-start gap-4">
          <Button variant="outline" size="icon" asChild className="shrink-0 h-10 w-10 rounded-full border-dashed">
          ...
          <div className="space-y-1.5 flex-1">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Badge variant="outline" className="text-xs font-medium uppercase tracking-wider py-1">
                {evaluacion.tipoEvaluacion.nombre}
              </Badge>
            </div>

            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{evaluacion.curso.nombre}</span>
              <span>•</span>
              <span className="flex items-center gap-1">
                {evaluacion.curso.nivelAcademico?.grado.nombre} "{evaluacion.curso.nivelAcademico?.seccion}"
              </span>
              <span>•</span>
              <span className="uppercase">{evaluacion.curso.areaCurricular.nombre}</span>
            </div>
          </div>
        </div>
          ...
          <CardContent className="p-4 sm:p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div className="flex flex-col gap-1.5 border-r last:border-0 border-border/50 pr-4">
                ...
                <span className="font-semibold text-foreground">{evaluacion.periodo.nombre}</span>
              </div>
              ...
              <div className="flex flex-col gap-1.5 border-r md:border-r last:border-0 border-border/50 pr-4">
                ...
                <span className="font-semibold text-foreground">{evaluacion.peso}% de la nota final</span>
              </div>
              ...
              <div className="flex flex-col gap-1.5 border-r last:border-0 border-border/50 pr-4">
                ...
                <span className="font-semibold text-foreground">
                  {evaluacion.notaMinima ? (
                    <span className="text-orange-500/90 dark:text-orange-400 font-bold">{evaluacion.notaMinima}</span>
                  ) : "N/A"}
                </span>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <span className="font-semibold text-foreground">{estudiantes.length} Alumnos</span>
              </div>
            </div>
          </CardContent>
        </Card>
