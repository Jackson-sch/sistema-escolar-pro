...
import { IconMail, IconPhone, IconMapPin, IconCertificate, IconEdit, IconBuilding, IconId } from "@tabler/icons-react"
...
export default function StaffProfile({ staff, showViewSheet, setShowViewSheet, setShowEditDialog }: StaffProfileProps) {
    ...
    <Sheet open={showViewSheet} onOpenChange={setShowViewSheet}>
      <SheetContent className="sm:max-w-[420px] p-0 overflow-y-auto bg-background flex flex-col">
        ...
        <SheetHeader className="sr-only">
          ...
          <SheetDescription>Expediente detallado de {staff.name} {staff.apellidoPaterno}</SheetDescription>
        </SheetHeader>
        ...
        <div className="relative w-full shrink-0">
          ...
          <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0]">
            <svg className="relative block w-full h-[20px]" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
              <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V120H0V95.8C57.1,118.92,156.63,69.08,321.39,56.44Z" className="fill-background opacity-50"></path>
            </svg>
          </div>
        ...
        <div className="flex-1 px-6 pt-14 pb-8 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-y-6 gap-x-4">
            <div className="space-y-1.5">
              ...
              <p className="text-sm font-semibold text-foreground font-mono">{staff.dni}</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <a href={`mailto:${staff.email}`} className="text-sm font-medium text-primary hover:underline block truncate">
                {staff.email}
              </a>
            </div>
            ...
            <div className="space-y-1.5">
               <div className="flex items-center gap-1.5 text-muted-foreground mb-0.5">
                ...
                <span className="text-[10px] uppercase font-bold tracking-wider">Teléfono</span>
              </div>
              <p className="text-sm text-foreground">{staff.telefono || <span className="italic opacity-50">No registrado</span>}</p>
            </div>

            <div className="space-y-1.5">
               <div className="flex items-center gap-1.5 text-muted-foreground mb-0.5">
                <IconMapPin className="size-3.5" />
                <span className="text-[10px] uppercase font-bold tracking-wider">Dirección</span>
              </div>
              <p className="text-sm text-foreground truncate">{staff.direccion || <span className="italic opacity-50">No registrada</span>}</p>
            </div>
          </div>
                  ...
                  <div className="relative z-10 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1 flex-1">
                        <p className="text-[10px] uppercase font-bold text-muted-foreground">Especialidad</p>
                        <p className="text-sm font-semibold text-foreground">{staff.especialidad || <span className="text-muted-foreground italic">No especificada</span>}</p>
                      </div>
                    ...
                    <div className="space-y-1">
                      <p className="text-[10px] uppercase font-bold text-muted-foreground">Grado Académico / Título</p>
                      <p className="text-sm text-foreground leading-snug">{staff.titulo || <span className="text-muted-foreground italic">Pendiente</span>}</p>
                    </div>
