...
import { PsychopedagogicalForm } from "./psychopedagogical-form"
import {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold tracking-tight">Registro Psicopedagógico</h3>
          <p className="text-xs text-muted-foreground font-medium">Seguimiento de conducta y bienestar emocional.</p>
        </div>
        <Button
          size="sm"
      ...
      <div className="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-linear-to-b before:from-transparent before:via-border/40 before:to-transparent">
        {loading ? (
              ...
              <div className="flex-1 bg-background/40 backdrop-blur-sm border border-border/40 rounded-2xl p-5 hover:border-emerald-500/20 transition-all group-hover:shadow-2xl group-hover:shadow-emerald-500/5">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <Badge variant="outline" className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-[9px] font-black uppercase tracking-widest px-2.5 h-5 shadow-none">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-[160px] border-border/40">
                      <DropdownMenuItem onClick={() => setEditingItem(item)} className="text-xs py-2 cursor-pointer">
                        <IconEdit className="mr-2 h-3.5 w-3.5 text-blue-500" /> Editar
                      </DropdownMenuItem>
                      <DropdownMenuItem
                ...
                <div className="space-y-3">
                  <h4 className="font-black text-sm uppercase tracking-tight text-foreground/90">{item.motivo}</h4>
                    ...
                    <div className="flex items-center gap-2">
                      <Avatar className="size-6 border border-border/40">
                        ...
                        <AvatarFallback className="text-[8px] bg-violet-500/10 text-violet-500 font-bold">
                          {item.especialista.name?.[0]}{item.especialista.apellidoPaterno?.[0]}
                        </AvatarFallback>
                      </Avatar>
                      <span className="text-[10px] text-muted-foreground/70 font-bold uppercase tracking-tighter">
                        Registrado por {item.especialista.name} {item.especialista.apellidoPaterno}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 border-2 border-dashed border-border/40 rounded-3xl bg-muted/5 relative z-10 mx-4">
            ...
            <p className="text-base font-black text-muted-foreground uppercase tracking-widest">Sin antecedentes</p>
            <p className="text-xs text-muted-foreground/60 mt-1.5 max-w-[200px] text-center font-medium">No se han registrado incidentes o seguimientos psicológicos aún.</p>
          </div>
        )}
      </div>
