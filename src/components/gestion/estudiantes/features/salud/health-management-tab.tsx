...
function getStudentHealthInitialData(student: StudentTableType) {
  return {
    ...
    restriccionesAlimenticias: student.restriccionesAlimenticias || undefined,
    centroSaludPreferido: student.centroSaludPreferido || undefined,
    peso: student.peso || undefined,
    talla: student.talla || undefined,
    parentescoContactoEmergencia: student.parentescoContactoEmergencia || undefined,
    nombreContactoEmergencia2: student.nombreContactoEmergencia2 || undefined,
    telefonoContactoEmergencia2: student.telefonoContactoEmergencia2 || undefined,
    parentescoContactoEmergencia2: student.parentescoContactoEmergencia2 || undefined,
...
function EstadoSaludCard({ student }: { student: StudentTableType }) {
  return (
    <Card className="relative overflow-hidden border border-border/60 bg-card rounded-2xl shadow-xs">
      <div className="p-0 border-b border-border/40 bg-emerald-500/5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 flex items-center gap-2 py-3.5 px-6">
...
function DatosPersonalesCard({ student }: { student: StudentTableType }) {
  const hasContactoEmergencia2 =
    Boolean(student.nombreContactoEmergencia2) || Boolean(student.telefonoContactoEmergencia2);

  return (
    <Card className="relative overflow-hidden border border-border/60 bg-card rounded-2xl shadow-xs">
      <div className="p-0 border-b border-border/40 bg-blue-500/5">
        <h3 className="text-xs font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 flex items-center gap-2 py-3.5 px-6">
      ...
      <div className="px-8 py-4 grid grid-cols-2 gap-4 text-sm">
        ...
        <div className="space-y-1">
          ...
          <p className="font-semibold">{student.lenguaMaterna || "ESPAÑOL"}</p>
        </div>
        <div className="space-y-1">
          <span className="text-[10px] uppercase font-bold text-muted-foreground/60 tracking-wider">
            Religión
      ...
      {hasContactoEmergencia2 && (
        <div className="p-4 border-t border-border/40 bg-orange-500/5">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold text-foreground/80">{student.nombreContactoEmergencia2}</p>
  ...
  return (
    <div className="space-y-6 animate-in fade-in-0 animation-duration-">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold tracking-tight">Salud y Bienestar</h3>
      ...
      <div className="grid grid-cols-1 gap-6">
        <EstadoSaludCard student={student} />
        <DatosPersonalesCard student={student} />
      </div>
      ...
      >
        <HealthForm
          ...
          initialData={getStudentHealthInitialData(student)}
