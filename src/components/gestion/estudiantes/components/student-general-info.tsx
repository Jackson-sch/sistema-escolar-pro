...
function AcademicaSection({
  nivelAcademico,
}: {
  nivelAcademico: StudentTableType["nivelAcademico"];
}) {
  return (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <div>
                <Badge
                  variant="outline"
                  className="bg-indigo-500/10 border-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-semibold text-xs rounded-lg"
                >
                  {nivelAcademico.nivel?.nombre || "General"}
                </Badge>
              </div>
            </div>
        ...
        {guardian ? (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/20 text-xs">
              ...
              {guardian.email && (
                <div className="flex items-center gap-1.5 text-muted-foreground font-mono col-span-2 min-w-0">
                  <IconMail className="size-3.5 text-muted-foreground/70 shrink-0" />
...
function ResidenciaSection({ student }: { student: StudentTableType }) {
  return (
    <div className="space-y-2.5">
      <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
...
function BiograficaSection({ student }: { student: StudentTableType }) {
  return (
...
export function StudentGeneralInfo({ student }: StudentGeneralInfoProps) {
  const primaryGuardianRelation =
    (student as any).padresTutores?.find((p: any) => p.contactoPrimario) ||
  ...
  return (
    <div className="space-y-5 animate-in fade-in-0 animation-duration-">
      <AcademicaSection nivelAcademico={student.nivelAcademico} />
      <GuardianSection
        student={student}
        guardian={guardian}
        primaryGuardianRelation={primaryGuardianRelation}
      />
      <ResidenciaSection student={student} />
      <BiograficaSection student={student} />
    </div>
  );
}
