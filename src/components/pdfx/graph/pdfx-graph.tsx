import { Circle, G, Line, Path, Rect, Svg, Text as SvgText } from "@/lib/pdf";
...
import type { ChartLayout, GraphProps, GraphSeries, GraphVariant } from './pdfx-graph.types';
import {
  GRAPH_SAFE_WIDTHS,
  arcPath,
  buildLayout,
...
function resolveChartContent({
  ...
  switch (variant) {
    case 'bar':
      return renderBarChart(series, layout, palette, showGrid, showValues, theme);
    case 'horizontal-bar':
    ...
    case 'pie':
      return renderPieDonutChart(series, layout, palette, undefined, false, theme);
    case 'donut':
      return renderPieDonutChart(series, layout, palette, centerLabel, true, theme);
    default:
      return null;
  }
}

function GraphAxesLabels({
  isPieOrDonut,
  xLabel,
  yLabel,
  chartX,
...
export function PdfGraph({
  ...
  const chartContent = resolveChartContent({
    variant,
    series,
    layout,
    palette,
