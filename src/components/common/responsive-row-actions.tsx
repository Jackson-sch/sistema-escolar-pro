    ...
    <div className={cn("flex items-center justify-end gap-1", className)}>
      ...
      <div className="md:hidden">
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-[180px] bg-background/95 border-border/40"
          >
            {label && (
            ...
            {mobileActions.map((action) => {
              if (action.isSeparator) {
                const sepKey = action.id ?? `separator-${action.variant || "default"}`;
                return (
                  <DropdownMenuSeparator key={sepKey} className="bg-border/40" />
                );
              }
              const Icon = action.icon;
              if (!Icon || !action.label || !action.onClick) return null;
