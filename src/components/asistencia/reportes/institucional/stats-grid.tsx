...
function AsistenciaStatCard({
  ...
  const badge = perc >= 90 ? "Óptimo" : perc >= 75 ? "Regular" : "Alerta";
  const color: StatColor = perc < 75 ? "rose" : perc < 90 ? "amber" : "emerald";
  ...
  return (
    <StatCard
      ...
      icon={IconSchool}
      badgeText={badge}
      subLabel={periodLabel || "Consolidado institucional"}
      color={color}
    />
  );
}

function PuntualidadStatCard({
  tasaPuntualidad,
  deltaTardanza,
}: {
  ...
  const badge = tasaPuntualidad >= 85 ? "A Tiempo" : "Tardanzas";
  const color: StatColor = tasaPuntualidad < 80 ? "rose" : tasaPuntualidad < 90 ? "amber" : "indigo";
  const delta = deltaTardanza ? -deltaTardanza : 0;
...
function AusentismoStatCard({ tasaAusentismo }: { tasaAusentismo: number }) {
  const badge = tasaAusentismo > 15 ? "Alto Riesgo" : "Bajo Ausentismo";
  const color: StatColor = tasaAusentismo > 20 ? "rose" : tasaAusentismo > 10 ? "amber" : "emerald";
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in animation-duration-">
      ...
      <PuntualidadStatCard
        ...
        deltaTardanza={stats?.deltaTardanza}
      />
      <AusentismoStatCard tasaAusentismo={tasaAusentismo} />
      <StatCard
