...
function CreateAnnouncementCard({ onClick }: { onClick?: () => void }) {
  return (
...
function AnnouncementAuthorFooter({ anuncio }: { anuncio: any }) {
  const authorName = anuncio.autor?.name || "Sistema";
  const authorLastName = anuncio.autor?.apellidoPaterno || "";
  const authorInitial = anuncio.autor?.name?.[0]?.toUpperCase() || "A";
...
function AnnouncementItemCard({ anuncio }: { anuncio: any }) {
  ...
  return (
    <>
      <article className="h-full">
        ...
        >
          <div className="group relative bg-card rounded-2xl overflow-hidden border border-border/60 shadow-xs hover:border-primary/40 transition-colors flex flex-col h-full w-full">
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <AnnouncementAuthorFooter anuncio={anuncio} />
            </div>
          </div>
        </div>
      </article>
...
export function AnnouncementCard({
  ...
  if (isCreateCard) {
    return <CreateAnnouncementCard onClick={onCreateClick} />;
  }

  return <AnnouncementItemCard anuncio={anuncio} />;
}
