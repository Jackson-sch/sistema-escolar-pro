...
import { EvaluacionStats, ActividadCursoChart, TiposEvaluacionChart } from "./reportes"
...
export function EvaluacionReports({ evaluaciones }: EvaluacionReportsProps) {
  ...
  const sinCalificar = evaluaciones.filter(ev => ev._count.notas === 0).length

  // Datos para gráfico de barras: Notas por Curso
  const cursosData = evaluaciones.reduce((acc: any[], ev) => {
  ...
  const tiposData = evaluaciones.reduce((acc: any[], ev) => {
    ...
    if (existing) {
      ...
      acc.push({ name: tipoName, value: 1, fill: PIE_COLORS[acc.length % PIE_COLORS.length] })
    }
    return acc
  }, [])

  // Generar config dinámico para el pie chart
  const dynamicPieConfig = tiposData.reduce((acc: ChartConfig, item: any, index: number) => {
    acc[item.name] = {
      label: item.name,
