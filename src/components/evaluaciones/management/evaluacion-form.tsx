...
function getEvaluacionDefaultValues(
  initialData: any,
  tipos: any[],
  periodos: any[],
  cursos: any[],
  ...
  return (
    <>
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="text-xs font-medium text-foreground/80">Curso</FormLabel>
          ...
          <FormItem>
            ...
            <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
              <FormControl>
                <SelectTrigger className="w-full bg-background border-border/40 rounded-xl text-xs h-9 font-medium">
                  <SelectValue
                    placeholder={loadingCapacidades ? "Cargando..." : "Seleccionar capacidad"}
                  />
                </SelectTrigger>
              </FormControl>
              <SelectContent className="rounded-xl border-border/40">
                {capacidades.length > 0 ? (
                  capacidades.map((cap) => (
                    <SelectItem key={cap.id} value={cap.id} className="text-xs font-medium">
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="text-xs font-medium text-foreground/80">Tipo de Evaluación</FormLabel>
            <Select onValueChange={field.onChange} defaultValue={field.value}>
              <FormControl>
                <SelectTrigger className="w-full bg-background border-border/40 rounded-xl text-xs h-9 font-medium">
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel className="text-xs font-medium text-foreground/80">Periodo Académico</FormLabel>
            <Select onValueChange={field.onChange} defaultValue={field.value}>
              <FormControl>
                <SelectTrigger className="w-full bg-background border-border/40 rounded-xl text-xs h-9 font-medium">
...
function EvaluacionFechaAndWeights({ form }: { form: any }) {
  return (
    <>
      <FormField
        control={form.control}
        ...
        render={({ field }) => (
          <FormItem className="flex flex-col">
            <FormLabel className="text-xs font-medium text-foreground/80">Fecha de Evaluación</FormLabel>
            <Popover>
      ...
      <div className="grid grid-cols-2 gap-2">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-xs font-medium text-foreground/80">Peso (%)</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  min={0}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className="text-xs font-medium text-foreground/80">Nota Mínima</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  min={0}
...
function EvaluacionFormActions({
  isPending,
  isEdit,
  onCancel,
}: {
      ...
      >
        {isPending ? (
          ...
          <>
            ...
            <span>{isEdit ? "Guardar Cambios" : "Crear Evaluación"}</span>
          </>
        )}
      </Button>
    </div>
...
export function EvaluacionForm({
  ...
  const form = useForm({
    defaultValues: getEvaluacionDefaultValues(initialData, tipos, periodos, cursos),
  });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 px-1 py-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          <EvaluacionSelectors
            ...
            loadingCapacidades={loadingCapacidades}
            tipos={tipos}
            periodos={periodos}
          />
          <EvaluacionFechaAndWeights form={form} />
        </div>

        <FormKeyboardHelpBar />

        <EvaluacionFormActions
          isPending={isPending}
          isEdit={!!initialData}
          onCancel={onSuccess}
        />
