 ...
 */
export async function getAnunciosAction(filters?: {
  nivelId?: string;
  gradoId?: string;
  activo?: boolean;
}) {
        ...
        AND: [
          filters?.nivelId
            ? {
                OR: [
                  { dirigidoA: "TODOS" },
                  { niveles: { some: { id: filters.nivelId } } },
                ],
              }
            : {},
          filters?.gradoId
            ? {
                OR: [
                  { dirigidoA: "TODOS" },
                  { grados: { some: { id: filters.gradoId } } },
                ],
              }
            : {},
 ...
 */
export async function getEventosAction(filters?: {
...
export async function upsertAnuncioAction(values: any, id?: string) {
  try {
    ...
    if (id) {
      ...
      const anuncio = await prisma.anuncio.update({
        ...
        data: {
          ...data,
          grados: grados
            ? { set: grados.map((id: string) => ({ id })) }
            : undefined,
          niveles: niveles
            ? { set: niveles.map((id: string) => ({ id })) }
            : undefined,
        },
      });
      ...
      return {
        ...
        data: JSON.parse(JSON.stringify(anuncio)),
      };
    } else {
      ...
      const anuncio = await prisma.anuncio.create({
        data: {
          ...
          autorId: user.id,
          grados: grados
            ? { connect: grados.map((id: string) => ({ id })) }
            : undefined,
          niveles: niveles
            ? { connect: niveles.map((id: string) => ({ id })) }
            : undefined,
        },
      });
    ...
    return {
      error: `No se pudo procesar el anuncio: ${error instanceof Error ? error.message : "Error desconocido"}`,
    };
  }
}
  ...
  try {
    ...
    if (id) {
      ...
      return {
        ...
        data: JSON.parse(JSON.stringify(evento)),
      };
    } else {
    ...
    return {
      error: `No se pudo procesar el evento: ${error instanceof Error ? error.message : "Error desconocido"}`,
    };
  }
}
