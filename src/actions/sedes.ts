...
export async function updateSedeAction(id: string, values: z.infer<typeof SedeSchema>) {
  try {
    const validatedFields = SedeSchema.safeParse(values)
...
export async function deleteSedeAction(id: string) {
  try {
    ...
    if (sede && sede._count.nivelesAcademicos > 0) {
      return { error: "No se puede eliminar la sede porque tiene niveles académicos asociados. Desactívela en su lugar." }
    }
